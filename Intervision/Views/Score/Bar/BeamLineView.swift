...
struct BeamLineView: View {
    ...
    let minimumPassThrough: CGPoint
    ...
    var beamStartPositions: [[CGPoint]] {
        ...
        let startStemTipPosition = CGPoint(x: furthestStartPosition.x, y: (direction == .Upward) ? furthestStartPosition.y - stemLength + stemLength / 50 : furthestStartPosition.y + stemLength - stemLength / 50)
        let endStemTipPosition = CGPoint(x: furthestEndPosition.x, y: (direction == .Upward) ? furthestEndPosition.y - stemLength + stemLength / 50 : furthestEndPosition.y + stemLength - stemLength / 50)
    ...
    var body: some View {
        ...
        ZStack {
            ...
            if let timeModificationValue = timeModificationValue {
                let numberPosition = CGPoint(x: midpointX, y: direction == .Upward ? midpointY - stemLength - 50 * scale : midpointY + stemLength + 50 * scale)
                ...
                if numberOfBeamLines.allSatisfy({ $0 == 0 }) {
                    let lineStart = CGPoint(x: furthestStartPosition.x + CGFloat(cos(backwardBeamAngle.radians)) * 20 * scale, y: direction == .Upward ? furthestStartPosition.y - stemLength - 25 * scale - CGFloat(sin(backwardBeamAngle.radians)) * 20 * scale : furthestStartPosition.y + stemLength + 25 * scale + CGFloat(sin(backwardBeamAngle.radians)) * 20 * scale)
                    let lineEnd = CGPoint(x: furthestEndPosition.x + CGFloat(cos(forwardBeamAngle.radians)) * 20 * scale, y: direction == .Upward ? furthestEndPosition.y - stemLength - 25 * scale - CGFloat(sin(forwardBeamAngle.radians)) * 20 * scale : furthestEndPosition.y + stemLength + 25 * scale + CGFloat(sin(forwardBeamAngle.radians)) * 20 * scale)
                    
                    let lineStartEdge = CGPoint(x: furthestStartPosition.x + CGFloat(cos(backwardBeamAngle.radians)) * 20 * scale, y: direction == .Upward ? furthestStartPosition.y - stemLength : furthestStartPosition.y + stemLength)
                    
                    let lineEndEdge = CGPoint(x: furthestEndPosition.x + CGFloat(cos(forwardBeamAngle.radians)) * 20 * scale, y: direction == .Upward ? furthestEndPosition.y - stemLength : furthestEndPosition.y + stemLength)
...
#Preview {
    BeamLineView(furthestStartPosition: CGPoint(x: 0, y: 0), furthestEndPosition: CGPoint(x: 0, y: 0), minimumPassThrough: CGPointZero, durations: [], timeModification: nil, stemLength: 10, direction: .Upward, scale: 1.0)
}
